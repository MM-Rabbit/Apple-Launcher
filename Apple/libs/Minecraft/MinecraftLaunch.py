    ...
    def launch_mc(launcher_version: str, appdata: str, ver: str, java_path: str, xmx: str, username: str, uuid: str,
        ...
        def is_new_json_f(ver_json_path: str) -> bool:  # 判断一个版本json文件是不是新格式
            try:
                with open(ver_json_path) as j:
                    ver_json: any = json.loads(j.read())
        ...
        if (
                    ...
                    for lib in ver_json["libraries"]:
                        if "classifiers" in lib["downloads"]:
                            for n in lib["downloads"]:  # 直接将全平台的库都解压了，懒得做平台检测了
                                if n == "artifact":
                                    dic_path: str = f"{appdata}/versions/{ver}/{ver}-natives"  # natives临时文件夹路径
                                    file_path: str = f"{appdata}/libraries/{lib['downloads'][n]['path']}"  # 库
                                    unzip(file_path, dic_path)
                                elif n == "classifiers":
                                    for native in lib["downloads"][n].values():
                                        dic_path: str = f"{appdata}/versions/{ver}/{ver}-natives"
                                        file_path: str = f"{appdata}/libraries/{native['path']}"  # classifiers' path（突然发癫（）
                                        unzip(file_path, dic_path)
                except Exception:
                    for lib in ver_json["libraries"]:
            ...
            for library in ver_json['libraries']:  # 某个Forge tm把json格式改了，我说怎么启动不了，上网一搜才知道，现在重写的是适配
                lib_list: list[str] = splitting_string_into_list_by_char(library["name"], ':')
                f_path: list = splitting_string_into_list_by_char(lib_list[0], '.')
                final_path: str = ''
                for p in f_path:
                    final_path += f"{p}/"
            ...
            classpath: list[str] = splitting_string_into_list_by_char(classpath, ';')
